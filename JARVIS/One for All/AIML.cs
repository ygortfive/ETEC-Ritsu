...
using System.IO;
using System.Xml; //processar arquivos xml
using AIMLbot; //processar aquivos aiml

namespace RITSU
{
    class AIML
    {
        private static string pathFiles = "DataBaseAIML"; //pasta dos arquivos aimls
        public static string[] GetWordsOrSentences()
        {
            string[] files = Directory.GetFiles(pathFiles, "*.aiml");
            List<string> wordsOrSentences = new List<string>();
        ...
        public static string GetOutputChat(string chat)
        {
            Bot myBot = new Bot();
            myBot.loadSettings();
            User user = new User("consoleUser", myBot);
